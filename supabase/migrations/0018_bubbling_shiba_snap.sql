-- I will execute the migration content here as a single block.
-- Since I cannot "run a migration file" via a tool, I execute the SQL directly.
...
CREATE OR REPLACE FUNCTION public.ensure_quotation_item_ownership() RETURNS trigger AS $$
DECLARE quote_user_id UUID;
BEGIN
  SELECT user_id INTO quote_user_id FROM public.quotations WHERE id = NEW.quotation_id;
  IF NEW.user_id <> quote_user_id THEN RAISE EXCEPTION 'Item user_id must match Quotation user_id'; END IF;
  RETURN NEW;
END; $$ LANGUAGE plpgsql;

CREATE TRIGGER tr_ensure_item_ownership BEFORE INSERT OR UPDATE ON public.quotation_items FOR EACH ROW EXECUTE PROCEDURE public.ensure_quotation_item_ownership();

CREATE OR REPLACE FUNCTION public.update_quotation_total() RETURNS trigger AS $$
DECLARE target_quote_id UUID;
BEGIN
  IF (TG_OP = 'DELETE') THEN target_quote_id := OLD.quotation_id; ELSE target_quote_id := NEW.quotation_id; END IF;
  UPDATE public.quotations SET total_amount = (SELECT COALESCE(SUM(line_total), 0) FROM public.quotation_items WHERE quotation_id = target_quote_id) WHERE id = target_quote_id;
  RETURN NULL;
END; $$ LANGUAGE plpgsql;
